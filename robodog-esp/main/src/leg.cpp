...
float Leg::tranY(float time)const{ //returns the Y value for every timestep
...
float Leg::bezier(const float p[5], float phase)const{ //returns the component of bezier curve which was calculated for the motion of a foot
...
float Leg::getKneeAngle(float time)const{ //return the knee angle for a given set of x,y in degree
    float x=tranX(time);
    float y=tranY(time);
...
void Leg::setStepHeight(float heightNew){ // in mm, should be positiv and in good relation to stepLength
    if(heightNew<=40.0f && heightNew<=0.5*stepLength && heightNew<=-0.5*height&& heightNew>0.0f){
        stepHeight = heightNew;
...
void Leg::setHeight(float heightNew){ // in mm, should be negativ between -80 and -135
    if(heightNew<=-80.0f && heightNew>=-155.0f){
        height=heightNew;
        bezierPointsY[0]=height;
        bezierPointsY[1]=height+0.1f*stepHeight;
        bezierPointsY[2]=height+3.0f*stepHeight;
