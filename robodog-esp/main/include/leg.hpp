...
class Leg {
    private:
        ...
        float tranX(float time)const; // calculates the x position of the foot at a given time
        float tranY(float time)const; // calculates the y position of the foot at a given time
        float bezier(const float p[5], float t)const; // calculates a point on a Bezier curve

    public:
        float getHipAngle(float time)const; // in degree
        float getKneeAngle(float time)const; // in degree
